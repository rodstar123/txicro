    ...
    <main className="min-h-screen bg-white">
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-20">
        ...
        {breadcrumbs && breadcrumbs.length > 0 && (
          <nav aria-label="Breadcrumb" className="mb-8">
            <ol className="flex items-center gap-2 text-sm text-[#3D4F5F]">
              <li><Link href={homeHref} className="hover:text-[#C9A84C] transition-colors">{homeLabel}</Link></li>
              {breadcrumbs.map((crumb, i) => (
                <li key={i} className="flex items-center gap-2">
                  ...
                  {i === breadcrumbs.length - 1 ? (
                    ...
                    <Link href={crumb.href} className="hover:text-[#C9A84C] transition-colors">{crumb.label}</Link>
                  )}
                </li>
              ))}
            </ol>
        ...
        <div className="text-center">
          <Link href={homeHref} className="inline-flex items-center text-[#C9A84C] hover:text-[#0A1628] transition-colors font-medium">
            {backHome}
          </Link>
        </div>
      </div>
